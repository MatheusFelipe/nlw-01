...
const Points = () => {
  ...
  useEffect(() => {
    ...
    Location.requestPermissionsAsync().then(({ status }) => {
      if (status !== 'granted') return Alert.alert('Oops!', 'Precisamos de sua permissão para obter a localização');
      return Location.getCurrentPositionAsync().then(({ coords: { latitude, longitude } }) =>
  ...
  useEffect(() => {
    api.get('points', { params: { city, uf, items: selectedItems } }).then(resp => setPoints(resp.data));
  }, [selectedItems]);

  const handleNavigateBack = () => navigation.goBack();
  const handleNavigateToDetail = (pointId: number) => navigation.navigate('Detail', { pointId });
